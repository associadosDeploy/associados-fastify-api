...
export async function updateAvatarAssociate(app: FastifyInstance) {
  app
    .withTypeProvider<ZodTypeProvider>()
    .register(auth)
    .patch(
      '/associates/avatar/:id',
      {
        schema: {
          tags: ['members'],
          summary: 'Change avatar profile of associate.',
          params: z.object({
            id: z.string(),
          }),
          security: [{ bearerAuth: [] }],
      ...
      async (request, reply) => {
        ...
        const checkCourseExists = await prisma.associate.findUnique({
          where: {
            id
          }
        })
